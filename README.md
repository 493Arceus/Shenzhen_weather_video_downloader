...
一个用于下载公开实景监测页面视频的 Python 脚本。

和旧版通过时间规律手工拼接视频文件名的方式不同，这个版本会直接读取页面实际使用的数据源，自动发现当前可用观测点及其最新视频地址，再执行下载或持续监控。
...
```bash
python main.py --keyword 示例站点 list
python main.py --keyword 示例站点 download
```
...
- 日期优先取自视频文件名中的时间戳，例如 `video_73_202604201119.mp4` 会归档到对应日期目录
